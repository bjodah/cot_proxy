...
def test_proxy_streaming_basic_think_tag_removal(client, mocker, caplog, enable_debug):
    ...
    with patch('cot_proxy.resolve_variant', return_value=pseudo):
        ...
        with patch('requests.request', return_value=mock_api_response) as mock_requests_call:
            request_body = {"model": "test-model@test", "stream": True, "messages": [{"role": "user", "content": "Hello stream"}]}
...
def test_proxy_streaming_llm_params_custom_tags(client, mocker, caplog):
    """Test streaming with custom think tags."""
    ...
    with patch('cot_proxy.resolve_variant', return_value=pseudo):
        ...
        with patch('requests.request', return_value=mock_api_response):
            request_body = {"model": "test-model@custom", "stream": True, "messages": [{"role": "user", "content": "Stream custom"}]}
            response = client.post("/v1/chat/completions", json=request_body)
...
def test_proxy_streaming_global_env_tags(client, mocker, caplog):
    ...
    variant = VariantConfig(
        ...
        inject_at_end='',
        weak_defaults={},
        thinking=ThinkingConfig(do_strip=True, tags=(env_start_tag, env_end_tag)),
        weak_logit_bias=[]
    )
    ...
    with patch('cot_proxy.resolve_variant', return_value=pseudo):
        ...
        with patch('requests.request', return_value=mock_api_response):
            request_body = {"model": "test-model@env", "stream": True, "messages": [{"role": "user", "content": "Stream env tags"}]}
            response = client.post("/v1/chat/completions", json=request_body)
        ...
        expected_log = f"Using think tags for model 'test-variant': START='{env_start_tag}', END='{env_end_tag}'"
        assert expected_log in caplog.text
...
def test_proxy_streaming_tag_split_across_chunks(client, mocker, caplog):
    ...
    with patch('cot_proxy.resolve_variant', return_value=pseudo):
        ...
        with patch('requests.request', return_value=mock_api_response):
            request_body = {"model": "test-model@split", "stream": True, "messages": [{"role": "user", "content": "Stream split"}]}
            response = client.post("/v1/chat/completions", json=request_body)
...
def test_proxy_streaming_client_disconnect(client, mocker, caplog):
    ...
    with patch('requests.request', return_value=mock_api_response) as mock_requests_call:
     ...
     # Assertions:
    # The underlying api_response.close() should have been called from the finally block.
    # Note: It may be called twice due to teardown_request
    mock_api_response.close.assert_called()
